...
use crate::{atom_as_command, atom_as_identifier};
...
pub fn console_fn(context: &mut Context, _head: &Atom, body: &[Atom]) -> Result<Signal, Backtrace> {
    ...
    for atom in body {
        let command = atom_as_command!(atom);
        let console_head = atom_as_identifier!(&command[0]);
        let console_args = &command[1..];

        let mut console_command = Command::new(console_head);
        for arg in console_args {
            console_command.arg(format!(
                "\"{}\"",
                context
                    .resolve_variant(arg)?
        ...
        let output = console_command.output();
